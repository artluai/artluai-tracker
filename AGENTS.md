...
Astro + Firebase Firestore + Netlify. Domain: artlu.ai.
...
- Slugs are generated by the MCP server on `add_project`, and regenerated when `name` changes. The old workaround of setting the slug by hand is no longer needed (fixed April 2026).
...
Rebuilt on Astro in May 2026. The old React + Vite single-page app is gone;
the `.jsx` files under `src/components/` are leftovers from it and are not
what the site renders. Pages are `.astro` under `src/pages/`.

**How the data actually flows.** Firestore is the source of truth; the site
is static.

1. Firestore project `artluai-tracker` holds `projects`, `journal`,
   `mainProjects`, `series`, `technologies`, and `siteMeta`.
2. `npm run sync:data` pulls every public document into
   `src/data/tracker-snapshot.json` (gitignored) using a service account key.
3. `npm run validate:stack` refuses to continue if any project's `stack`
   label has no matching record in `technologies`.
4. `astro build` renders the snapshot to static pages; Netlify publishes
   `dist`.

So a change in Firestore is invisible until a build runs. A build runs on a
push to main, or from the daily `publish-if-dirty` job in the
artlu-tracker-mcp repo.

**Routes**: `/` stack spectrum, `/list` timeline, `/log` journal
(`/journal` 301s here), `/research`, `/map`, `/project/:slug`, `/video/:id`,
`/admin` (`/dashboard` 301s here).
