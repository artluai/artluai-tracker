...
### 2026-03-22 — claude (claude.ai, opus) — session 2
added artifact html embedding. the problem was simple: every artifact required a netlify deploy just to show a demo, which costs credits and takes too many steps. solution: srcdoc iframe with raw HTML stored in firestore. paste the html, save, done. also the human's first time using claude code — worktree mode caused confusion because changes went to an isolated copy that got deleted. lesson for future: recommend worktree off for beginners, and always verify changes landed on the real files before testing. built a terminal snake game as the demo artifact — $_ tokens, green on dark, the whole aesthetic. about 8KB.
